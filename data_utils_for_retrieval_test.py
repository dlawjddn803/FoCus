...
def build_input_from_segments_for_retrieval_performance_test(persona, knowledge, history, persona_cans, persona_grounding, knowledge_cans, knowledge_answer_idx, ID, tokenizer, lm_labels=False, testset=False, inference=False, with_eos=True):
    ...
    gt_knowledge = [tokenizer.decode(knowledge_cans[knowledge_answer_idx])]
    top1_knowledge = [tokenizer.decode(chosen_knowledge[0])]
    #print('gt: ', gt_knowledge, 'top1: ', top1_knowledge)
    bert_score = cacluate_bertscore_with_gt(gt_knowledge, top1_knowledge)
    ...
    instance['bert_score'] = bert_score
    ...
    for dataset_name, dataset in plan.items():
        ...
        for dialog in dataset:
            ...
            for i, utt in enumerate(utterance):
                ...
                elif model_name =='BART' and args.flag == 'retrieval_test':
                    total_utt += 1
                    instance = build_input_from_segments_for_retrieval_performance_test(persona, knowledge, history, persona_cans, persona_grouding,
                                                                     knowledge_cans, knowledge_answer_idx, ID, tokenizer,
                                                                     lm_labels=True, testset=testset, inference=args.inference)
                    #print('bert_score', bert_score)
                    #print('instance bert: ', instance['bert_score'])


                for input_name, input_array in instance.items():
                    if input_name == 'bert_score':
                        bert_score += instance['bert_score']
                    else:
                        datasets[dataset_name][input_name].append(input_array)

        print('total: ', total_utt, 'bert_score: ', bert_score, 'avg_bert_score: ', bert_score/total_utt)
